if php -m | grep oci8; then
	...
	sudo phpenmod -v 7.2 oci8
	sudo service php7.2-fpm restart
fi

if cat /etc/php/7.2/fpm/php-fpm.conf | grep ORACLE_HOME; then
	echo 'php-fpm oracle env path already set!'
else
	echo "env[ORACLE_HOME] = '/u01/app/oracle/product/11.2.0/xe'" >> /etc/php/7.2/fpm/php-fpm.conf
	echo "env[LD_LIBRARY_PATH] = '/u01/app/oracle/product/11.2.0/xe/lib'" >> /etc/php/7.2/fpm/php-fpm.conf
fi
